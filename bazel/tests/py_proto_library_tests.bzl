...
def _test_python_proto2_deps_impl(env, target):
    ...
    is_upb = "net/proto2/python/public/message.py" not in transitive_sources
    if is_upb:
        env.expect.that_collection(transitive_sources).contains("third_party/py/google/protobuf/upb/__init__.py")
    else:
